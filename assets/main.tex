...
%   [大图] 图片必须独占一页（[plain,c] 全幅帧），标题直接 \figcap：
%          \begin{frame}[plain,c]
%            \centering
%            \includegraphics[height=0.92\textheight,width=\textwidth,keepaspectratio]{...}
%            \figcap{N}{标题}   ← \figcap 内置 \par\smallskip\centering，无需额外 \tightgap
%          \end{frame}
%          禁止在图片下方追加 block / callout / 说明文字；图片独占一页。
%   [指标] \metric 必须把 \times 等数学符号包在 $...$ 中（如 $10\times$）。
%   [度数] 用 $^\circ$ 而非 \textdegree（后者需 textcomp 包）。
% =============================================================================
% =============================================================================
% 幻灯片比例配置（只改 aspectratio 的数值即可）
%   aspectratio=1610  16:10  实验室调研 / 论文分享统一比例   ← 默认
%   aspectratio=169   16:9   宽屏会议/投影（需要时切换）
%   aspectratio=43    4:3    老式投影 / 教室白板
%   其它可选：149 (14:9) · 54 (5:4) · 32 (3:2) · 1410 (14:10)
% 标题页与各排版宏均按相对宽度自适应，切换比例无需改正文。
% =============================================================================
\documentclass[aspectratio=1610,10pt,fontset=none]{ctexbeamer}
...
\setCJKmonofont{NotoSansCJKsc-Regular.otf}[Path=fonts/,BoldFont=NotoSansCJKsc-Bold.otf]

\usetheme{sustech}
\hidecredits
% Keep colored headers aligned in paragraph (X) table columns.
\renewcommand{\thc}[1]{{\leavevmode\color{thc}\bfseries #1}}
\setlogo[0.12\paperheight]{ntu-logo.png}
\newcommand{\presenterinstitute}{南洋理工大学 / Nanyang Technological University}
\titlegraphic{{\scriptsize\presenterinstitute\par}\vspace{0.4em}\includegraphics[height=0.12\paperheight,keepaspectratio]{ntu-logo.png}}
        % Madrid 基础 + 学术配色 + 排版宏（见 sustech-theme/）

% 可视化分析分节页（\vizbgon 触发）依赖的条件声明；论文分享默认关闭：
\newif\ifvizbgdiv
\vizbgdivfalse
...
% --- 元信息（替换为你的题目 / 作者）------------------------------------------
% 标题页字段（新版结构，见 sustech-theme 说明）：
%   主标题 / 副标题 → 论文全部作者 → 团队所属单位 →（论文来源）→ 领域 →
%   汇报人 · 地点 · 时间 → logo
\title[〈短标题〉]{〈中文主标题〉}
\subtitle{〈英文副标题 / English subtitle〉}
\author[〈一作 et al.〉]{〈论文全部作者（按论文署名顺序）〉}
\institute[〈机构〉]{〈团队所属单位〉}
\setsource{〈会议 / 期刊〉}{〈年份〉}   % 论文来源；不需要可删除本行
\setdomains{\domaintag{〈领域1〉}\domaintag{〈领域2〉}\domaintag{〈领域3〉}}
\setpresenter{〈汇报人 / Presenter〉}
\setvenue{NTU}
\date{\today}
  ...
  \twopane{%
    ...
    \begin{block}{核心观察}
      \begin{itemize}\setlength{\itemsep}{0.15em}\footnotesize
        \item \shl{\ph{关键数字或特性一}}
        \item \shl{\ph{关键数字或特性二}}
        \item \ph{核心观察三}
        \item \ph{核心观察四}
      \end{itemize}
    \end{block}
    ...
    \begin{block}{附录亮点}
      \begin{itemize}\setlength{\itemsep}{0.1em}\footnotesize
        \item \shl{\ph{亮点一}}：\ph{简要描述}
        \item \shl{\ph{亮点二}}：\ph{简要描述}
        \item \shl{\ph{亮点三}}：\ph{简要描述}
      \end{itemize}
    \end{block}
    \tightgap
    \begin{callout}[一句话概括]
...
% \begin{frame}[plain,c]
%   \centering
%   \includegraphics[height=0.92\textheight,width=\textwidth,keepaspectratio]{fig1.jpg}
%   \figcap{1}{\ph{图注}}
% \end{frame}
...
% \begin{frame}[plain,c]
%   \centering
%   \includegraphics[height=0.92\textheight,width=\textwidth,keepaspectratio]{fig2.jpg}
%   \figcap{2}{\ph{系统 / 方法框图}}
% \end{frame}
...
% \begin{frame}[plain,c]
%   \centering
%   \includegraphics[height=0.92\textheight,width=\textwidth,keepaspectratio]{fig3.jpg}
%   \figcap{3}{\ph{任务图示}}
% \end{frame}
...
% \begin{frame}[plain,c]
%   \centering
%   \includegraphics[height=0.92\textheight,width=\textwidth,keepaspectratio]{fig4.jpg}
%   \figcap{4}{\ph{曲线 / 分析图}}
% \end{frame}
